...
Operational instructions for coding agents working on the **Lanes** project.
...
- Game type: Lane-based tactical RPG inspired by [Grand Kingdom](https://en.wikipedia.org/wiki/Grand_Kingdom).
...
| Gameplay behavior (combat, lane movement, abilities, UI flow) | Global Verification Gate + DevTools runtime loop: `ping`, relevant `input` simulation, `screenshot`, `validate-all`, `performance`, `input clear` |
...
### Screenshot Expectations for Combat Changes

- Lane switching: capture before and after unit changes lane.
- Attack execution: capture the attack animation/effect frame.
- Turn transitions: capture turn order UI update.
- AoE abilities: capture the area indicator and effect.
- UI changes: capture the updated HUD/menu state.
...
python tools/devtools.py input tap attack
python tools/devtools.py input tap lane_up
python tools/devtools.py input sequence test/sequences/combat_template.json
...
- `dotnet test`: pure C# logic tests (combat math, turn order, stats, lane logic).
